    ...
    <div className="min-h-[80vh] flex flex-col items-center justify-center max-w-2xl mx-auto">
      ...
      <div className="w-full mb-6">
        <div className="relative">
          ...
          <input
            ...
            placeholder="Enter company name... (e.g. McDonald's, Tesla, Slack)"
            disabled={loading}
        ...
        <div className="mt-3 flex items-center gap-2 text-xs px-4 py-2 rounded-lg border text-emerald-600 bg-emerald-50 border-emerald-100">
          <CheckCircle size={14} />
          <span>Ready to research — <strong>No API key needed!</strong> Uses Wikipedia + curated database. Unlimited & free.</span>
        </div>
      </div>
      ...
      {error && (
        <div className="w-full mb-6 bg-red-50 border border-red-100 rounded-xl p-4 flex items-start gap-3">
          ...
          <button onClick={() => setError('')} className="text-xs text-red-500 underline ml-auto">Dismiss</button>
        </div>
      )}
          ...
          <div className="space-y-2">
            {pastResearches.slice(0, 5).map((r, i) => (
              ...
              >
                ...
                <span className="text-[10px] text-slate-300 ml-auto">{new Date(r.timestamp).toLocaleDateString()}</span>
              </button>
            ))}
          </div>
        </div>
