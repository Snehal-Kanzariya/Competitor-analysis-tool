 ...
 */
export async function researchCompany(companyName, apiKey = null, onProgress = () => {}) {
...
function parseResearchResult(companyName, competitorRaw, analysisRaw) {
  ...
  if (competitors.company) {
    allCompanies.push({
      ...
      opportunities: analysis?.swot?.[competitors.company.name]?.opportunities || [],
  ...
  if (competitors.competitors) {
    competitors.competitors.forEach((comp, i) => {
      allCompanies.push({
        ...
        score: Math.max(40, Math.min(95, 75 - i * 5 + Math.floor(Math.random() * 10))),
        trend: ['+5', '+2', '-1', '+8', '+3'][i % 5],
  ...
  if (analysis?.scoring) {
    ...
    allCompanies.forEach((comp) => {
      scoringScores[comp.id] = analysis.scoring.scores?.[comp.name] || scoringCriteria.map(() => 5);
    });
  }

  // Build positioning data
  let positioning = { xAxis: 'Price Level', yAxis: 'Product Quality', positions: {} };
  if (analysis?.positioning) {
    ...
    allCompanies.forEach((comp) => {
      positioning.positions[comp.id] = analysis.positioning.positions?.[comp.name] || {
        x: 30 + Math.random() * 40,
        y: 30 + Math.random() * 40,
      };
    });
...
export function getDemoResearch(companyName) {
  return {
    ...
    competitors: [
      {
        id: 'main', name: companyName, website: '', industry: 'Technology',
        founded: '', hq: '', employees: '', description: `${companyName} - researched company`,
        marketShare: '25%', score: 80, trend: '+5', status: 'Target', color: '#3B82F6',
      ...
      ...[1, 2, 3, 4, 5].map((i) => ({
        ...
        score: 75 - i * 5,
        trend: ['+5', '+2', '-1', '+8', '+3'][i - 1],
        status: ['Leader', 'Contender', 'Follower', 'Rising', 'Contender'][i - 1],
    ...
    monopoly: {
      ...
      disruptionOpportunities: ['AI-first approach', 'Underserved segments', 'Better pricing'],
