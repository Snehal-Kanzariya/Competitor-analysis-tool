...
const sampleCompetitors = [
  {
    id: '1', name: 'Acme Corp', website: 'acmecorp.com', industry: 'SaaS',
    founded: '2019', hq: 'San Francisco, CA', employees: '250-500',
    funding: 'Series C · $45M', description: 'Enterprise analytics platform.',
    estimatedRevenue: '$12M ARR', marketShare: '15%',
    score: 82, trend: '+5', status: 'Leader', color: '#3B82F6',
  ...
  {
    id: '2', name: 'BetaWorks', website: 'betaworks.io', industry: 'SaaS',
    founded: '2020', hq: 'Austin, TX', employees: '100-250',
    funding: 'Series B · $20M', description: 'Developer-focused tools.',
    estimatedRevenue: '$6M ARR', marketShare: '8%',
    score: 74, trend: '+2', status: 'Contender', color: '#8B5CF6',
  ...
  {
    id: '3', name: 'ClearView', website: 'clearview.co', industry: 'SaaS',
    founded: '2018', hq: 'London, UK', employees: '50-100',
    funding: 'Series A · $8M', description: 'Budget-friendly SMB solution.',
    estimatedRevenue: '$3M ARR', marketShare: '5%',
    score: 68, trend: '-1', status: 'Follower', color: '#F59E0B',
  ...
  {
    id: '4', name: 'DataPrime', website: 'dataprime.dev', industry: 'SaaS',
    founded: '2022', hq: 'Berlin, DE', employees: '20-50',
    funding: 'Seed · $3M', description: 'AI-first analytics startup.',
    estimatedRevenue: '$1M ARR', marketShare: '2%',
    score: 61, trend: '+8', status: 'Rising', color: '#10B981',
  ...
  {
    id: '5', name: 'Your Product', website: 'yourproduct.com', industry: 'SaaS',
    founded: '2024', hq: 'Remote', employees: '1-10',
    funding: 'Bootstrapped', description: 'Your competitive analysis tool.',
    estimatedRevenue: '$0', marketShare: '0%',
    score: 77, trend: '+12', status: 'You', color: '#EC4899',
...
const sampleMonopoly = {
  ...
  disruptionOpportunities: ['AI-first approach', 'Underserved SMB segment', 'Better pricing model'],
...
const useStore = create((set, get) => {
  ...
  return {
    ...
    positioning: saved?.positioning || { xAxis: 'Price Level', yAxis: 'Product Quality', positions: {} },
    ...
    loadResearch: (result) => {
      set((state) => {
        const pastResearches = [
          ...
          ...(state.pastResearches || []).filter((r) => r.query !== result.query),
        ].slice(0, 20);
        ...
        const updated = {
          ...
          positioning: result.positioning || state.positioning,
          pastResearches,
    ...
    addCompetitor: (competitor) => {
      const newCompetitor = {
        ...
        trend: '+0', status: 'New',
        color: ['#3B82F6', '#8B5CF6', '#F59E0B', '#10B981', '#EF4444', '#EC4899'][get().competitors.length % 6],
      ...
      set((state) => {
        ...
        state.features.forEach((f) => { newFeatureScores[newCompetitor.id][f.id] = 'no'; });
        const updated = { ...state, competitors: [...state.competitors, newCompetitor], featureScores: newFeatureScores };
        saveData(getState(updated));
        return updated;
      });
    },
    ...
    updateCompetitor: (id, updates) => {
      set((state) => {
        const updated = { ...state, competitors: state.competitors.map((c) => c.id === id ? { ...c, ...updates } : c) };
        saveData(getState(updated));
        return updated;
      });
    },
    ...
    deleteCompetitor: (id) => {
      set((state) => {
        ...
        delete newFS[id];
        const updated = { ...state, competitors: state.competitors.filter((c) => c.id !== id), featureScores: newFS };
        saveData(getState(updated));
        return updated;
      });
    },
    ...
    addFeature: (feature) => {
      ...
      set((state) => {
        ...
        const updated = { ...state, features: [...state.features, newFeature], featureScores: newFS };
        saveData(getState(updated));
        return updated;
      });
    },
    ...
    updateFeature: (id, updates) => {
      set((state) => {
        const updated = { ...state, features: state.features.map((f) => f.id === id ? { ...f, ...updates } : f) };
        saveData(getState(updated));
        return updated;
      });
    },
    ...
    deleteFeature: (id) => {
      set((state) => {
        ...
        const updated = { ...state, features: state.features.filter((f) => f.id !== id), featureScores: newFS };
        saveData(getState(updated));
        return updated;
      });
    },
    ...
    resetToSample: () => {
      const data = {
        competitors: sampleCompetitors, features: sampleFeatures,
        featureScores: sampleFeatureScores, monopoly: sampleMonopoly,
        marketOverview: sampleMarketOverview,
        pastResearches: get().pastResearches, currentResearchQuery: '',
      };
      set(data);
      saveData(data);
    },
